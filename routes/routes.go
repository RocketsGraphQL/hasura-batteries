...
func ChiFacebookCallback(w http.ResponseWriter, r *http.Request) {
	...
	redirectUrI := os.Getenv("FACEBOOK_REDIRECT_FINAL_URL") + "?rgraphRefreshToken=" + refreshTokenCookie.Value + "&rgraphAccessToken=" + accessTokenCookie.Value
	render.Status(r, http.StatusCreated)
...
func ChiFacebookSecretsSet(w http.ResponseWriter, r *http.Request) {
	...
	os.Setenv("FACEBOOK_REDIRECT_URI", data.RedirectURL)
	os.Setenv("FACEBOOK_REDIRECT_FINAL_URL", data.FinalRedirectURL)
...
func ChiFacebookClient(w http.ResponseWriter, r *http.Request) {
	githubRedirectUrl := os.Getenv("GITHUB_REDIRECT_URL")
	providerUrl := "https://www.facebook.com/v14.0/dialog/oauth?client_id=856950301942789&redirect_uri=http://localhost:7000/api/facebook/callback&state={'{st=state123abc,ds=123456789}'}"
	log.Println("github details: ", githubRedirectUrl, providerUrl)
